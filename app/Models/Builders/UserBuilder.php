...
class UserBuilder extends Builder
{
    ...
    {
        if ($inMobile) {
            return $this->whereNotNull('email')->where('email', $data['email'])->whereIn('type', UserTypeHelper::mobileTypes());
        }
        return $this->whereNotNull('email')->where('email', $data['email'])->whereIn('type', UserTypeHelper::nonMobileTypes());
    }
    ...
    // public function getUniqueColumnName(): string
    // {
    //     return 'email';
    // }
}
